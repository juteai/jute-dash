...
func TestNormalizeLayoutDefaultsModeAndEnforces12Col(t *testing.T) {
	catalog := repository.WidgetCatalogForSeed()
	layout := model.WidgetLayout{
		ProfileID: model.DefaultLayoutProfileID,
		Widgets: []model.WidgetInstance{
			{ID: "w1", Kind: "date-time", Title: "Clock", X: 0, Y: 0, W: 6, H: 1, MinW: 3, MinH: 1, Size: "wide"},
		},
	}
	normalized, err := repository.NormalizeWidgetLayout(layout, catalog)
	if err != nil {
		t.Fatalf("normalize: %v", err)
	}
	if normalized.Widgets[0].Mode != model.WidgetModeUI {
		t.Fatalf("expected default mode ui, got %q", normalized.Widgets[0].Mode)
	}

	headless := model.WidgetLayout{
		ProfileID: model.DefaultLayoutProfileID,
		Widgets: []model.WidgetInstance{
			{
				ID:    "w1",
				...
				MinH:  1,
				Size:  "wide",
				Mode:  model.WidgetModeHeadless,
			},
		},
	}
	if _, err := repository.NormalizeWidgetLayout(headless, catalog); err != nil {
		t.Fatalf("headless mode should be valid: %v", err)
	}

	tooWide := model.WidgetLayout{
		ProfileID: model.DefaultLayoutProfileID,
		Widgets: []model.WidgetInstance{
			{ID: "w1", Kind: "date-time", Title: "Clock", X: 8, Y: 0, W: 6, H: 1, MinW: 3, MinH: 1, Size: "wide"},
		},
	}
	if _, err := repository.NormalizeWidgetLayout(tooWide, catalog); err == nil {
		t.Fatalf("expected column-bounds error for x+w > %d", model.BaseColumns)
	}
}
