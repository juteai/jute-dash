...
func (r *DashboardRepository) SetOnSave(onSave func(ctx context.Context)) {
	r.onSave = onSave
}

func (r *DashboardRepository) SetConfigStore(cs any) {
	r.configStore = cs
}

func (r *DashboardRepository) ConfigStore() any {
	return r.configStore
}

func (r *DashboardRepository) SetCatalog(items []WidgetCatalogItem) {
...
func (r *DashboardRepository) CatalogByKind() map[string]WidgetCatalogItem {
	return r.catalog
}

func (r *DashboardRepository) WidgetLayout(
	ctx context.Context,
	profileID string,
) (WidgetLayout, error) {
	...
	for _, w := range wDBs {
		...
		if strings.TrimSpace(w.SettingsJSON) != "" {
			if err := json.Unmarshal([]byte(w.SettingsJSON), &widget.Settings); err != nil {
				return WidgetLayout{}, fmt.Errorf(
					"decode widget settings for %s: %w",
					widget.ID,
					err,
				)
			}
		...
		if strings.TrimSpace(w.ConnectionRefsJSON) != "" {
			if err := json.Unmarshal([]byte(w.ConnectionRefsJSON), &widget.ConnectionRefs); err != nil {
				return WidgetLayout{}, fmt.Errorf(
					"decode widget connection refs for %s: %w",
					widget.ID,
					err,
				)
			}
...
func (r *DashboardRepository) SaveWidgetLayout(
	ctx context.Context,
	layout WidgetLayout,
) (WidgetLayout, error) {
	...
	err = r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&LayoutProfileDB{}).
			Where("id = ?", normalized.ProfileID).
...
func (r *DashboardRepository) SetActiveScreen(
	ctx context.Context,
	profileID string,
	screenID string,
) (WidgetLayout, error) {
...
func (r *DashboardRepository) ResetWidgetLayout(
	ctx context.Context,
	profileID string,
) (WidgetLayout, error) {
...
func normalizeLayoutVariant(variant LayoutVariant, widgets []WidgetInstance) LayoutVariant {
	...
	for _, widget := range widgets {
		...
		variant.Placements[widget.ID] = clampWidgetPlacement(
			placement,
			widget,
			variant.Columns,
			variant.Rows,
...
func validateScreenVariants(screen DashboardScreen) error {
	...
	for _, variant := range screen.Variants {
		...
		if seen[variant.ID] {
			return fmt.Errorf(
				"%w: duplicate layout variant %q on screen %q",
				ErrInvalidLayout,
				variant.ID,
				screen.ID,
			)
		...
		if variant.Columns < minLayoutGridSize || variant.Columns > maxLayoutGridSize ||
			...
			return fmt.Errorf(
				"%w: layout variant %q grid size is invalid",
				ErrInvalidLayout,
				variant.ID,
			)
		}
		...
		default:
			return fmt.Errorf(
				"%w: layout variant %q orientation is invalid",
				ErrInvalidLayout,
				variant.ID,
			)
		}
...
func WidgetCatalog() []WidgetCatalogItem {
	...
	catalog := make(map[string]WidgetCatalogItem, len(fallback)+len(widgets.List()))
	order := make([]string, 0, len(fallback)+len(widgets.List()))
	for _, item := range fallback {
		catalog[item.Kind] = item
		order = append(order, item.Kind)
	}
	for _, registered := range widgets.List() {
		item := registered.CatalogInfo()
		if _, exists := catalog[item.Kind]; !exists {
			order = append(order, item.Kind)
		}
		catalog[item.Kind] = item
	}
	out := make([]WidgetCatalogItem, 0, len(order))
	for _, kind := range order {
		out = append(out, catalog[kind])
	}
	return out
}
...
func validateWidgetInstance(widget WidgetInstance) error {
	if widget.X < 0 || widget.Y < 0 {
		return fmt.Errorf(
			"%w: widget %s position must be non-negative",
			ErrInvalidLayout,
			widget.ID,
		)
	}
	...
	if widget.W < widget.MinW || widget.H < widget.MinH {
		return fmt.Errorf(
			"%w: widget %s is smaller than its minimum size",
			ErrInvalidLayout,
			widget.ID,
		)
	}
	if widget.W > BaseColumns || widget.MinW > BaseColumns || widget.X+widget.W > BaseColumns {
		return fmt.Errorf(
			"%w: widget %s exceeds dashboard column bounds",
			ErrInvalidLayout,
			widget.ID,
		)
	}
	...
	default:
		return fmt.Errorf(
			"%w: widget %s has unsupported mode %q",
			ErrInvalidLayout,
			widget.ID,
			widget.Mode,
		)
	...
	default:
		return fmt.Errorf(
			"%w: widget %s has unsupported size %q",
			ErrInvalidLayout,
			widget.ID,
			widget.Size,
		)
