...
func (r *HomeRepository) SetOnSave(onSave func(ctx context.Context)) {
	r.onSave = onSave
}

func (r *HomeRepository) SetupStatus(ctx context.Context) (SetupStatus, error) {
...
func (r *HomeRepository) HouseholdSettings(ctx context.Context) (HouseholdSettings, error) {
	var hs HouseholdSettingsDB
	if err := r.db.WithContext(ctx).First(&hs, "id = ?", DefaultHouseholdID).Error; err != nil {
...
func (r *HomeRepository) Rooms(ctx context.Context) ([]RoomConfig, error) {
	return r.loadRooms(ctx)
}

func (r *HomeRepository) SaveRooms(ctx context.Context, rooms []RoomConfig) ([]RoomConfig, error) {
...
func (r *HomeRepository) Tiles(ctx context.Context) ([]TileConfig, error) {
	return r.loadTiles(ctx)
}

func (r *HomeRepository) SaveTiles(ctx context.Context, tiles []TileConfig) ([]TileConfig, error) {
...
func (r *HomeRepository) SaveHouseholdSettings(
	ctx context.Context,
...
func (r *HomeRepository) AdapterConnections(ctx context.Context) ([]AdapterConnection, error) {
...
func (r *HomeRepository) AdapterConnection(
	ctx context.Context,
	id string,
	...
	if id == "" {
		return AdapterConnection{}, fmt.Errorf(
			"%w: adapter connection id is required",
			ErrInvalidSettings,
		)
	}
...
func (r *HomeRepository) SaveAdapterConnection(
	ctx context.Context,
...
func adapterConnectionFromDB(row AdapterConnectionDB) (AdapterConnection, error) {
	...
	if strings.TrimSpace(row.SettingsJSON) != "" {
		if err := json.Unmarshal([]byte(row.SettingsJSON), &settings); err != nil {
			return AdapterConnection{}, fmt.Errorf(
				"decode adapter connection settings for %s: %w",
				row.ID,
				err,
			)
		}
	...
	if strings.TrimSpace(row.SecretRefJSON) != "" {
		if err := json.Unmarshal([]byte(row.SecretRefJSON), &secretRefs); err != nil {
			return AdapterConnection{}, fmt.Errorf(
				"decode adapter connection secret refs for %s: %w",
				row.ID,
				err,
			)
		}
...
func (r *HomeRepository) loadRooms(ctx context.Context) ([]RoomConfig, error) {
	var dbRooms []RoomDB
...
func (r *HomeRepository) loadTiles(ctx context.Context) ([]TileConfig, error) {
	var dbTiles []TileDB
...
func (r *HomeRepository) IsSeeded(ctx context.Context) (bool, error) {
	var count int64
