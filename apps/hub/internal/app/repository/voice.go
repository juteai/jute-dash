...
func (r *VoiceRepository) VoiceSettings(
	ctx context.Context,
...
func (r *VoiceRepository) SaveVoiceSettings(
	ctx context.Context,
...
func (r *VoiceRepository) SetVoiceMuted(
	ctx context.Context,
	deviceProfileID string,
	muted bool,
) (Settings, error) {
...
func (r *VoiceRepository) CancelVoice(
	ctx context.Context,
	deviceProfileID string,
) (Settings, error) {
	deviceProfileID = strings.TrimSpace(deviceProfileID)
...
func (r *VoiceRepository) VoiceProviders(ctx context.Context) ([]ProviderPack, error) {
...
func (r *VoiceRepository) ActiveWakeProvider(
	ctx context.Context,
...
func (r *VoiceRepository) ActiveSTTProvider(
	ctx context.Context,
	deviceProfileID string,
) (STTProvider, error) {
	settings, err := r.VoiceSettings(ctx, deviceProfileID)
	...
	if manifest.Transport.Type == "command" {
		if !settings.CommandProvidersEnabled ||
			strings.TrimSpace(manifest.Transport.Command) == "" {
			return nil, nil
		}
...
func (r *VoiceRepository) TTSVoices(
	ctx context.Context,
	providerID, deviceProfileID string,
) (TTSVoicesResponse, error) {
...
func (r *VoiceRepository) ActiveTTSProvider(
	ctx context.Context,
	deviceProfileID string,
) (TTSProvider, error) {
	settings, err := r.VoiceSettings(ctx, deviceProfileID)
...
func (r *VoiceRepository) EnsureDefaultVoiceSettings(ctx context.Context, voice Config) error {
	now := nowUTC()
	var count int64
	if err := r.db.WithContext(ctx).
		Model(&SettingsDB{}).
