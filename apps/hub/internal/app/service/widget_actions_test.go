...
func (s fakeLayoutStore) WidgetLayout(_ context.Context, _ string) (WidgetLayout, error) {
...
func TestDispatcherValidatesActionRequests(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			dispatcher: NewWidgetActionDispatcher(fakeLayoutStore{layout: WidgetLayout{}}, successResolver, nil),
			request:    WidgetActionRequest{WidgetInstanceID: "missing", ActionID: "do"},
			wantStatus: http.StatusNotFound,
			wantIssue:  "widget.not_found",
		},
		{
			name: "removed instance",
			dispatcher: NewWidgetActionDispatcher(fakeLayoutStore{layout: WidgetLayout{
				Widgets: []WidgetInstance{{
					ID:      "widget-1",
					Kind:    testWidgetKind,
					Visible: false,
			...
			request:    WidgetActionRequest{WidgetInstanceID: "widget-1", ActionID: "do"},
			wantStatus: http.StatusNotFound,
			wantIssue:  "widget.not_found",
		},
		{
			name:       "missing action",
			dispatcher: NewWidgetActionDispatcher(fakeLayoutStore{layout: layout}, successResolver, nil),
			request:    WidgetActionRequest{WidgetInstanceID: "widget-1", ActionID: "missing"},
		...
		},
		{
			...
			dispatcher: NewWidgetActionDispatcher(fakeLayoutStore{layout: layout}, successResolver, nil),
			request:    WidgetActionRequest{WidgetInstanceID: "widget-1", ActionID: "confirm"},
			wantStatus: http.StatusConflict,
			wantIssue:  "widget.action_confirmation_required",
		},
		{
			name: "connection issue",
			dispatcher: NewWidgetActionDispatcher(fakeLayoutStore{layout: layout}, fakeConnectionResolver{
				result: widgets.ConnectionResolution{
...
func TestDispatcherInvokesConnectionAwareWidget(t *testing.T) {
	...
	dispatcher := NewWidgetActionDispatcher(
		...
		func(_ context.Context, layout WidgetLayout) widgetskills.Snapshot {
			return widgetskills.Snapshot{Layout: widgetskills.WidgetLayout{ProfileID: layout.ProfileID}}
		},
	)

	result := dispatcher.Dispatch(context.Background(), WidgetActionRequest{
