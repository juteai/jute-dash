// Code generated by mockery v2.53.6. DO NOT EDIT.

package mocks

import (
	context "context"
	model "jute-dash/apps/hub/internal/app/model"

	mock "github.com/stretchr/testify/mock"
)

// Syncer is an autogenerated mock type for the Syncer type
type Syncer struct {
	mock.Mock
}

type Syncer_Expecter struct {
	mock *mock.Mock
}

func (_m *Syncer) EXPECT() *Syncer_Expecter {
	return &Syncer_Expecter{mock: &_m.Mock}
}

// AgentsConfig provides a mock function with given fields: ctx
func (_m *Syncer) AgentsConfig(ctx context.Context) ([]model.AgentConfig, error) {
	ret := _m.Called(ctx)

	if len(ret) == 0 {
		panic("no return value specified for AgentsConfig")
	}

	var r0 []model.AgentConfig
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context) ([]model.AgentConfig, error)); ok {
		return rf(ctx)
	}
	if rf, ok := ret.Get(0).(func(context.Context) []model.AgentConfig); ok {
		r0 = rf(ctx)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]model.AgentConfig)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context) error); ok {
		r1 = rf(ctx)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Syncer_AgentsConfig_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'AgentsConfig'
type Syncer_AgentsConfig_Call struct {
	*mock.Call
}

// AgentsConfig is a helper method to define mock.On call
//   - ctx context.Context
func (_e *Syncer_Expecter) AgentsConfig(ctx interface{}) *Syncer_AgentsConfig_Call {
	return &Syncer_AgentsConfig_Call{Call: _e.mock.On("AgentsConfig", ctx)}
}

func (_c *Syncer_AgentsConfig_Call) Run(run func(ctx context.Context)) *Syncer_AgentsConfig_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context))
	})
	return _c
}

func (_c *Syncer_AgentsConfig_Call) Return(_a0 []model.AgentConfig, _a1 error) *Syncer_AgentsConfig_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *Syncer_AgentsConfig_Call) RunAndReturn(run func(context.Context) ([]model.AgentConfig, error)) *Syncer_AgentsConfig_Call {
	_c.Call.Return(run)
	return _c
}

// SyncAgents provides a mock function with given fields: ctx, configs
func (_m *Syncer) SyncAgents(ctx context.Context, configs []model.AgentConfig) error {
	ret := _m.Called(ctx, configs)

	if len(ret) == 0 {
		panic("no return value specified for SyncAgents")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, []model.AgentConfig) error); ok {
		r0 = rf(ctx, configs)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// Syncer_SyncAgents_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'SyncAgents'
type Syncer_SyncAgents_Call struct {
	*mock.Call
}

// SyncAgents is a helper method to define mock.On call
//   - ctx context.Context
//   - configs []model.AgentConfig
func (_e *Syncer_Expecter) SyncAgents(ctx interface{}, configs interface{}) *Syncer_SyncAgents_Call {
	return &Syncer_SyncAgents_Call{Call: _e.mock.On("SyncAgents", ctx, configs)}
}

func (_c *Syncer_SyncAgents_Call) Run(run func(ctx context.Context, configs []model.AgentConfig)) *Syncer_SyncAgents_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].([]model.AgentConfig))
	})
	return _c
}

func (_c *Syncer_SyncAgents_Call) Return(_a0 error) *Syncer_SyncAgents_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *Syncer_SyncAgents_Call) RunAndReturn(run func(context.Context, []model.AgentConfig) error) *Syncer_SyncAgents_Call {
	_c.Call.Return(run)
	return _c
}

// NewSyncer creates a new instance of Syncer. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewSyncer(t interface {
	mock.TestingT
	Cleanup(func())
}) *Syncer {
	mock := &Syncer{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
