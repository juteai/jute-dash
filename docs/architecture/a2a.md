...
### Safe Agent Request Proxying
To allow the display frontend to utilize A2A JS libraries (like `a2a-js`) while strictly keeping credentials inside the hub, the hub will implement a proxy router at `/api/v1/proxy/agents/{agentId}/*`.
- The display app sends standard A2A JSON-RPC/HTTP requests to this proxy endpoint.
- Jute Hub intercepts the request, looks up credentials (such as Bearer tokens) configured for `{agentId}`, injects them into the outgoing request's authorization headers, and forwards the request to the agent's endpoint.
