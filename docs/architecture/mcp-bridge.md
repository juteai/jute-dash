...
The bridge exposes hub-approved dashboard context and hub-mediated display actions. It does not expose raw widget internals, direct widget iframe access, raw adapter payloads, or credentials.
...
- expose local Jute dashboard and widget context as resources;
- expose safe Jute actions as tools;
- expose reusable guidance as prompts;
- notify connected local agents when available context or tools change;
...
The bridge exposes resources for context. Resource contents are generated by the hub at read time and are safe public projections.
...
The bridge exposes tools for safe hub-mediated actions.

Initial tools:

- `jute_dashboard_context_get`: return the same safe context as `jute://dashboard/current`.
- `jute_widget_list`: list visible widgets available to the agent.
- `jute_widget_read_context`: read public context for one visible widget.
...
The bridge exposes one initial prompt:

- `jute_home_assistant_guidance`

The prompt explains how an agent should use Jute context safely:

- inspect available resources and tools before assuming capabilities;
- prefer visible widget context over hidden or guessed state;
...
The prompt is guidance, not a permission grant.
...
The MCP Bridge uses the same widget context policy as A2A dashboard context.

Widgets may contribute context only when:

- the widget declares the public field;
...
2. Add config and SQLite fields for MCP settings and per-agent scopes.
3. Add a read-only MCP bridge with resources and context tools.
4. Add local token auth and loopback transport.
5. Add display mutation tools.
6. Add notifications.
7. Add future approval-gated home action tools.
