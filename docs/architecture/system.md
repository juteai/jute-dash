...
- `/api/v1/tts/speak`, `/api/v1/tts/stop`: current speech-policy and transient TTS state controls; command providers may synthesize or perform local playback while the hub exposes only safe state metadata.
