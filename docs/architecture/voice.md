...
provider only when voice is enabled, the manifest is a local/offline Wyoming STT pack, required
credentials are satisfied, and provider health is `available` or `degraded`. Public provider
summaries continue to omit transport endpoints and credential metadata.
