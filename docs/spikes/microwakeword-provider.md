...
replaces `{fixture}` and `{fixtureId}` in `-wake-command-args`, removes the temporary file after the
...
go run ./apps/hub/cmd/jute-voice-benchmark \
  ...
  -wake-command-args-json '["detect", "--model", "okay-nabu", "--fixture-id", "{fixtureId}", "--input", "{fixture}", "--json"]' \
  -provider-id pmdroid-microwakeword \
  -model-id okay-nabu \
  -model-hash sha256:replace-with-model-hash \
