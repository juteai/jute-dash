// Command ollama-a2a exposes an Ollama-backed ADK agent through A2A 1.0.
package main
...
func run() error {
	...
	ollamaBaseURL := strings.TrimSpace(os.Getenv("OLLAMA_API_BASE"))
	if ollamaBaseURL == "" {
		ollamaBaseURL = "http://localhost:11434/v1"
	}

	modelID := strings.TrimSpace(os.Getenv("OLLAMA_MODEL"))
	if modelID == "" {
	...
	// Initialize Ollama Model client using the official adk-go-ollama provider
	parsedURL, err := url.Parse(ollamaBaseURL)
	if err != nil {
		return fmt.Errorf("invalid OLLAMA_API_BASE URL: %w", err)
	}
	llm, err := ollama.New(modelID, ollama.WithBaseURL(parsedURL))
	if err != nil {
	...
	a, err := llmagent.New(llmagent.Config{
		Name:        "ollama_a2a_assistant",
		Description: "A helpful assistant running on a local Ollama model and exposed over A2A.",
		Model:       llm,
	...
	args := os.Args[1:]
	if len(args) == 0 {
		args = []string{"web", "a2a", "--port", "9999", "--a2a_agent_url", "http://localhost:9999"}
	}
