...
func homeAssistantGuidance() string {
	return "Use Jute MCP resources and Widget Skills to understand only the visible dashboard context that the hub exposes. Prefer skill context over guesses. Invoke only declared actions. Do not infer hidden widget state, secrets, private household data, camera frames, microphone audio, browser storage, or raw adapter payloads. If context is missing or unauthorized, say so briefly and continue the A2A conversation without it."
}
